...
class TestCleanupRoutePermissions(unittest.TestCase):
    ...
    def test_admin_seer_import_run_awaits_coroutine(self):
        ...
        with patch(
            "api_service.blueprints.cleanup.routes.execute_seer_import_job",
            new=AsyncMock(return_value=result),
        ) as execute_seer_import_job:
            resp = self.client.post("/api/cleanup/seer-import/run", json={"dry_run": True})

        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.get_json(), {"status": "success", "result": result})
        execute_seer_import_job.assert_awaited_once_with(force_run=True, override_dry_run=True)
